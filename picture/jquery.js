...
jQuery.fn = jQuery.prototype = {
	...
	//转换为数组调用数组的slice方法将自身转换为数组，其他类型转换为空数组，数组类型返回自身
	toArray: function() {
	...
	/**获取函数
	 * 对于是null对象返回自身的数组形式
	 * 对于是其他类型对于num的值小于0返回负值加上数据长度是值，对于大于0的值返回对应位置上的值
	 */
	get: function( num ) {
	...
	//压栈的实现，将传入的数据转换为数组且设置其的preObject对象。并返回
	pushStack: function( elems ) {

		// Build a new jQuery matched element set
		//this.construceor函数实际为JQuery.fn.init函数
...
jQuery.extend( {
	...
	each: function( obj, callback ) {
		...
		/**
		 * 对于是类数组的处理
		 * 获取数组的长度，遍历数组，使用回调函数调用，如果遇到flase退出循环
		 * 对于不是类数组的处理遍历对象的内容并调用回调函数，如果遇到flase退出循环
		 */
	...
	text: function( elem ) {
		...
		/**
		 *根据节点类型进行处理
		 如果节点类型的值为0，处理后面的元素对象
		 如果节点类型为1（元素），9（文档类型），11（轻量级文档类型）的处理，如果元素的文本内容的类型为字符串，返回此字符串反之返回使用此方法变量元素的所有的子元素
		 如果节点类型为3（文本内容），4（CDATA）内容返回节点的值
		 */
		if ( !nodeType ) {
	...
	/**
	 * 根据回调函数的返回值如果为真添加到数组中否则不添加
	 * @param {*} elems 元素对象
	 * @param {*} callback 回调函数
	 * @param {*} invert 是否进行反向过滤
	 */
	...
	/**
	 * 根据回调函数的执行进行判断如果结果不为null或者是undefined添加到数组中
	 * @param {*} elems 元素对象
	 * @param {*} callback  回调函数
	 * @param {*} arg 参数
	 */
