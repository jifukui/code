 ...
 */
/**判断对象是否是原始对象 string number boolean */
...
function logError (err, vm, info) 
{
  ...
  /**对于有console不为未定义的处理输出错误信息和log没有区别 */
...
function checkComponents (options) 
{
  for (var key in options.components) 
  {
    ...
    /**这部分主要是检测传入的option是否存在slot,component 
     * config$1的isReservedTag方法始终返回否
    */
...
{
  console.log("initUse "+JSON.stringify(this));
  Vue.use = function (plugin) 
  {
    console.log("initUse use "+JSON.stringify(this));
...
function initMixin$2 (Vue) 
{
  ...
  {
    console.log("inittMixin$2 mixin "+JSON.stringify(this));
...
/**判断第6个字符是否为:号且前5个字符为xlink */
var isXlink = function (name) 
{
  return name.charAt(5) === ':' && name.slice(0, 5) === 'xlink'
};
/**对于name为xlink:返回剩下的内容否则返回空字符串 */
...
function setAttr (el, key, value) 
{
  ...
  if (isBooleanAttr(key)) 
  {
    ...
    /**对于参数为真的处理 获取value的值为key是否为允许全屏且标签名称为EMBED
     * 设置此属性的值为value的值
    */
    else 
    {
...
/**对此节点开始寻找父节点和子节点直至没有进行动态类和静态类的拼接 */
...
{
  var dynamicValue =getAndRemoveAttr$1(el, ':' + name) ||getAndRemoveAttr$1(el, 'v-bind:' + name);
  if (dynamicValue != null) 
  {
    return parseFilters(dynamicValue)
  } 
  ...
  generateComponentTrace$1 = function (vm) 
  {
    if (vm._isVue && vm.$parent) 
    {
      ...
      return '\n\nfound in\n\n' + tree
        .map(function (vm, i) 
        { 
          return ("" + (i === 0 ? '---> ' : repeat$1(' ', 5 + i * 2)) + (Array.isArray(vm)
...
{
  console.log("createCompilerCreator template "+JSON.stringify(template));
  console.log("createCompilerCreator options "+JSON.stringify(options));
...
{
  el = el && query$1(el);
  console.log("Vue$3.prototype.$mount ");
  /* istanbul ignore if */
