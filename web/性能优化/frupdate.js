    ...
    if (ligObject.updatafile != "") 
    {
        ...
        $("#HttpCommUploadIFrame").on("submit", function () 
        {
                if (sendfile == 1) 
                {
                    ...
                    jifukui_dialog();
                    //showDialogBox(true, true, "Warning", "<div>To upgrade your device image, click OK.</div><div>Do not interrupt the file transfer before completion. Doing so may damage the device</div>", "");
                    /*}
                    else if (filename == 0) {
                        $('#kDialogBtnCancel').hide();
                        $('#kDialogBtnOk').show();
